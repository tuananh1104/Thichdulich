...
export class AIRecommendationService {
  ...
  static findSimilarUsers(
    ...
    const currentUserBookings = allBookings.filter(b => b.userId === currentUserId);
    if (currentUserBookings.length === 0) return [];

    const currentTourIds = new Set(currentUserBookings.map(b => b.tourId));
    ...
    const userSimilarities = allUsers
      ...
      .map(user => {
        ...
        const userTourIds = new Set(userBookings.map(b => b.tourId));

        // Jaccard similarity
        const intersection = [...currentTourIds].filter(id => userTourIds.has(id)).length;
        const union = new Set([...currentTourIds, ...userTourIds]).size;
        const similarity = union > 0 ? intersection / union : 0;

        return { userId: user.id, similarity };
      })
      .filter(item => item.similarity > 0.2) // Threshold
      .sort((a, b) => b.similarity - a.similarity)
      .slice(0, limit);

    return userSimilarities.map(item => item.userId);
  }

  /**
   * Gợi ý dựa trên collaborative filtering
   */
  static getCollaborativeRecommendations(
    userId: string,
    allTours: Tour[],
    allBookings: Booking[],
    allUsers: any[],
    ...
    const similarUsers = this.findSimilarUsers(userId, allBookings, allUsers, 10);
    if (similarUsers.length === 0) return [];

    const currentUserTourIds = new Set(
      allBookings.filter(b => b.userId === userId).map(b => b.tourId)
    );

    // Lấy tours mà similar users đã book nhưng current user chưa
    const recommendedTourIds = new Map<string, number>(); // tourId -> count

    similarUsers.forEach(simUserId => {
      const userBookings = allBookings.filter(b => b.userId === simUserId);
      userBookings.forEach(booking => {
        if (!currentUserTourIds.has(booking.tourId)) {
          recommendedTourIds.set(
            booking.tourId,
            (recommendedTourIds.get(booking.tourId) || 0) + 1
          );
        }
      });
    });
    ...
    const sortedTourIds = Array.from(recommendedTourIds.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, limit)
      .map(([tourId]) => tourId);

    return sortedTourIds
